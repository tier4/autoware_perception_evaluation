...
def sum_clear(clears: List[CLEAR]) -> Dict[str, Any]:
    """Sums up multiple CLEAR results.

    Args:
        clears (List[CLEAR]): List of CLEARs.

    Returns:
        Dict[str, Any]: MOTA, MOTP and IDSW.
    """
    mota: float = 0.0
    motp: float = 0.0
    num_gt: int = 0
    ...
    return {"MOTA": mota, "MOTP": motp, "IDSW": id_switch}
