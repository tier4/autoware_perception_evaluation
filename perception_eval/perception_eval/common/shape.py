...
class ShapeType(Enum):
    ...
    def from_value(cls, name: str) -> ShapeType:
        ...
        for k, v in cls.__members__.items():
            if v == name:
                return k
        msg = f"Unexpected name: {name}, choose from {list(cls.__members__.keys())}"
        raise ValueError(msg)

    def __str__(self) -> str:
        return self.value

    def __eq__(self, other: str | ShapeType) -> bool:
...
class Shape:
    ...
    def __init__(
        ...
        self.footprint: Polygon | None = footprint if footprint else self.__calculate_corners(shape_type, size)
    ...
    def __calculate_corners(shape_type: ShapeType, size: tuple[float, float, float]) -> Polygon:
        ...
        if shape_type != ShapeType.BOUNDING_BOX:
            msg = f"Expected BOUNDING_BOX, but got {shape_type}, which should have footprint."
            raise ValueError(msg)
