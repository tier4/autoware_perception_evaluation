...
class DynamicObject:
    ...
    def get_corners(self, scale: float = 1.0) -> np.ndarray:
        """[summary]
        Get the bounding box corners with respect ot base_link or map coordinate system.

        Args:
            scale (float): Scale factor to scale the corners. Defaults to 1.0.

        Returns:
            corners (numpy.ndarray): Object's corners with respect to base_link or map coordinate system.
        """
